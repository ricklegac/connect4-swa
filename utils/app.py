...
class Board(object):
    ...
    def place(self, jugador, col):
        assert(jugador == self.PLAYER1 or jugador == self.PLAYER2)
...
class App(tk.Frame):
    ...
    def __init__(self, alg_fn_map, master=None):
        ...
        self.master.title("Busqueda Adversario -- Conecta4")
    ...
    def new_game(self):
        ...
        listbox_player2 = tk.OptionMenu(self.new_game_window, player2_var, "IA", "Persona")
    ...
    def draw_checker(self, jugador, x, y, tag=""):
        c = "#c40003" if jugador == self.PLAYER1 else "white"
    ...
    def run_game(self, player1, search_depth1, player2, search_depth2, search_fn):
        ...
        def place(jugador, col, render=True):
            if self.board.place(jugador, col):
                print("Player {} places at the {} column".format(1 if jugador == self.PLAYER1 else 2, ordinal(col+1)))
                print(self.board.dump())
                print("############################################")
                if render:
                    row = next(r for r in range(self.board.rows) if self.board.occupied(r, col))
                    self.draw_checker(jugador, 
                        board_pos[0]+(col+0.05)*self.cell_size,
                        board_pos[1]+(row+0.05)*self.cell_size
                    )
                    self.canvas.update()
                return self.PLAYER2 if jugador == self.PLAYER1 else self.PLAYER1
            if jugador == "Human":
                return jugador
            raise ValueError("IA intenta poner en una columna no valida ({})".format(col))

        def human_motion(jugador, x, y, tag="ultimo_movimiento_persona"):
            x -= board_pos[0]
            col = int(x/self.cell_size)
            y -= board_pos[1]
            row = int(y/self.cell_size)
            self.canvas.delete("ultimo_movimiento_persona")
        ...
        def human_click(jugador, x, y):
            loc = human_motion(jugador, x, y, "")
            if loc is not None:
                adversary = place(jugador, loc, render=False)
                self.canvas.unbind("<Motion>")
                self.canvas.unbind("<1>")
                return turn_for(adversary)

        def turn_for(jugador):
            ...
            if winner is not None:   
                ...
                return
            agent = player1 if jugador == self.PLAYER1 else player2
            if agent == "Human":
                self.canvas.bind("<Motion>", lambda e: human_motion(jugador, e.x, e.y))
                self.canvas.bind("<1>", lambda e: human_click(jugador, e.x, e.y))
                x = self.canvas.winfo_pointerx()-self.canvas.winfo_rootx()
                y = self.canvas.winfo_pointery()-self.canvas.winfo_rooty()
                human_motion(jugador, x, y)
            else:
                print("############################################")
                if agent == "Random":
                    m = []
                    ...
                    action, *_ = search_fn(jugador, self.board, search_depth1 if jugador == self.PLAYER1 else search_depth2)
                if action is None:
                    self.prompt("jugador {} no juega ".format(1 if jugador == self.PLAYER1 else 2))
                    print("jugador {} no juega".format(1 if jugador == self.PLAYER1 else 2))
                else:
                    adversary = place(jugador, action)
